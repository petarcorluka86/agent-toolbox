...
| **`connect‑yt`** | Tells the agent it *can* reach YouTrack (the URL + token are already in `commands/.env`) and shows how: source the env, prove the connection with `/api/users/me`, then hit the REST API with the Bearer token. For when an agent wrongly assumes it lacks YouTrack access. |
